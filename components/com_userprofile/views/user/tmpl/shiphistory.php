...
<?php
    ...
    foreach($ordersHistoryView as $rg){
       ...
       //  <!-- old feature start -->
        ...
        for($i=0 ; $i < count($rg->ItemDetails) ;$i++){
          ...
          //echo '<tr id="'.str_replace(" ","",$rg->BillFormNo).'" style="display:block;" ><td></td><td></td><td>'.$rg->ItemDetails[$i]->ItemName.'</td><td>'.$rg->ItemDetails[$i]->ItemQuantity.'</td><td>'.$rg->ItemDetails[$i]->ItemStatus.'</td><td></td></tr>';
                  echo '<tr id="'.$rg->BillFormNo.'" style="display:none" ><td></td><td>'.$rg->ItemDetails[$i]->ItemName.'</td><td>'.$rg->ItemDetails[$i]->ItemQuantity.'</td><td>'.$rg->ItemDetails[$i]->ItemStatus.'</td><td></td><td></td></tr>';

        }  
        
        echo '<tr ><td class="action_btns"><input type="button" class="btn btn-success wrchild" data-id="'.$rg->BillFormNo.'" value="+"></td><td><a data-toggle="modal" data-backdrop="static" data-keyboard="false" class="getmlog whrse-link label-success" data-id="'.$rg->BillFormNo.'" data-target="#logModal" title="momento">'.$rg->BillFormNo.'</a></td><td>'.$rg->CreatedDate.'</td><td>'.$rg->CarrierName.'</td><td>'.$rg->TrackingId.'</td><td>'.$rep.'</td></tr>';
