...
class UserprofileControllerUser extends JControllerLegacy
{
    ...
    //for($idk=0;$idk<count($invIdkArr);$idk++){
        ...
        // new code
//         $fileNameStr = 'invFile_'.($idk+1);
//         $mulfiles[$invIdkArr[$idk]] = JRequest::getVar($fileNameStr, null, 'files', 'array');
...
//         $extensionStrNew="";
//         $fileNameStr = 'invFile_'.($idk+1);
//         $profilepicFile = JRequest::getVar($fileNameStr, null, 'files', 'array');
//         foreach($_FILES[$fileNameStr]['name'] as $key=>$mage){
//             $profilepicname =JFile::makeSafe($_FILES[$fileNameStr]['name'][$key]);
//             $photodest = JPATH_SITE. "/media/com_userprofile/".$TARGET.'/'.$profilepicname;
           
//             if($_FILES[$fileNameStr]["tmp_name"][$key]){
//                 JFile::upload($_FILES[$fileNameStr]["tmp_name"][$key], $photodest);
//             }
...
//             $imageByteStream = base64_encode($image1);
//             $invf .= $imageByteStream.',';
//             $filenameStrNew .= $profilepicname.',';
//             $extensionStr ='.'.pathinfo($profilepicname, PATHINFO_EXTENSION );
//             $extensionStrNew = $extensionStr.',';
//             $invIdkNewStr = $invIdkArr[$idk].",";
            
//             }else{
//                 $invf .= '0,';
//             } 
            
//             $mulfiles[$invIdkArr[$idk]] = array($invIdkNewStr,$invf,$filenameStrNew,$extensionStrNew);
...
// 	var_dump($httpResponse);exit;
	...
	public function get_ajax_data()
	{
        ...
        if($paymenttype!="" &&  $shippmentflag!=""){
           ...
           echo UserprofileHelpersUserprofile::getShippmentDetails($user,$paymenttype,$wherhourec,$invidk,$qty,$dt[0],$volres,$mr[0],$sr[0],$shiptype,$services,$dvalue,$bustype,$lengthStr,$widthStr,$heightStr,$grosswtStr,$volumeStr,$volumetwtStr,$dvalueStr,$shipmentCost,$destCnt,$rateType);
           exit;
        }
